---
title: "Full Likelihood EIV Model: Analytical vs Numerical Gradient Benchmark (bench::mark)"
author: "aishell"
output:
  html_document:
    keep_md: true
...
fit_full <- function(fn, data, use_grad = TRUE) {
  ...
  optim(start, fn, data = data, method = 'BFGS', hessian = TRUE, compute_grad = use_grad)
}

if (!requireNamespace('bench', quietly = TRUE)) install.packages('bench', repos='https://cloud.r-project.org')
library(bench)

bench_results <- bench::mark(
  Analytical = fit_full(negloglik_vec_grad, dat, use_grad = TRUE),
  Numerical  = fit_full(negloglik_vec_grad, dat, use_grad = FALSE),
  iterations = 3,
  check = FALSE
)
bench_results
